...
interface ImageCacheActions {
  ...
  cacheImageDimensions: (
    imageId: number,
    dimensions: Omit<ImageDimensionsCache, "lastAccessed">
  ) => void;

  /**
   * 批量缓存图片尺寸信息
   */
  batchCacheImageDimensions: (
    items: Array<{
      imageId: number;
      dimensions: Omit<ImageDimensionsCache, "lastAccessed">;
    }>
  ...
  cacheImagePreview: (
    imageId: number,
    preview: Omit<ImagePreviewCache, "lastAccessed">
  ) => void;

  /**
   * 批量缓存图片预览数据
   */
  batchCacheImagePreviews: (
    items: Array<{
      imageId: number;
      preview: Omit<ImagePreviewCache, "lastAccessed">;
    }>
  ...
  cacheImageMetadata: (
    imageId: number,
    metadata: Omit<ImageMetadataCache, "lastAccessed">
  ) => void;
 ...
 * 图片缓存管理 Store
 */
export const useImageCacheStore = create<ImageCacheState & ImageCacheActions>(
  (set, get) => ({
    // 初始状态
    ...
    getCacheStats: () => {
      ...
      const totalMemoryUsage =
        (dimensionsCache.size + previewCache.size + metadataCache.size) *
        avgImageDataSize;

      // 计算命中率
      const totalHits =
        stats.dimensionsHitCount +
        stats.previewHitCount +
        stats.metadataHitCount;
      const hitRate =
        stats.totalRequests > 0 ? (totalHits / stats.totalRequests) * 100 : 0;
    ...
    shouldCleanup: () => {
      ...
      // 检查内存使用量是否超过限制
      const totalLimit =
        config.thumbnailCacheSize +
        config.dimensionsCacheSize +
