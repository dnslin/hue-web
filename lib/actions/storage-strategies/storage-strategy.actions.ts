// lib/actions/storage-strategies/storage-strategy.actions.ts
"use server"
...
const STORAGE_STRATEGIES_API_BASE = "/admin/storage-strategies"
...
export async function getStorageStrategiesAction(
  ...
  try {
    ...
    const response = await apiService.get<PaginatedApiResponse<StorageStrategy>>(
...
export async function createStorageStrategyAction(
  ...
  try {
    ...
    const requestData = {
      ...
      ...(data.type === 'local' && data.localConfig && {
        local_config: {
...
> {
  try {
    ...
    console.error("getUserAvailableStorageStrategiesAction 错误:", error.message)
